...
public class SubjectController extends BaseController {
	...
	/*ajax方式按照查询条件分页查询科目信息*/
	@RequestMapping(value = { "/list" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void list(Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if(rows != 0)subjectService.setRows(rows);
		List<Subject> subjectList = subjectService.querySubject(page);
	    /*计算总的页数和总的记录数*/
	    subjectService.queryTotalPageAndRecordNumber();
	    /*获取到总的页码数目*/
	...
	public String frontlist(Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		...
		List<Subject> subjectList = subjectService.querySubject(currentPage);
	    /*计算总的页数和总的记录数*/
	    subjectService.queryTotalPageAndRecordNumber();
	    /*获取到总的页码数目*/
	    int totalPage = subjectService.getTotalPage();
	...
	public String delete(@PathVariable Integer subjectId,HttpServletRequest request) throws UnsupportedEncodingException {
		  try {
			  subjectService.deleteSubject(subjectId);
	            request.setAttribute("message", "科目删除成功!");
	            return "message";
	        } catch (Exception e) { 
	            e.printStackTrace();
	            request.setAttribute("error", "科目删除失败!");
				return "error";
	...
	public void OutToExcel( Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        ...
        String[] headers = { "科目名称","适用车辆","驾照级别"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<subjectList.size();i++) {
        	Subject subject = subjectList.get(i); 
        	dataset.add(new String[]{subject.getSubjectName(),subject.getCarType(),subject.getDriveLevel()});
        }
        /*
