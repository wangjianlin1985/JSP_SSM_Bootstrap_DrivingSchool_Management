...
public class CoachController extends BaseController {
	...
	public void list(String name,String birthDate,String telephone,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		List<Coach> coachList = coachService.queryCoach(name, birthDate, telephone, page);
	    /*计算总的页数和总的记录数*/
	    coachService.queryTotalPageAndRecordNumber(name, birthDate, telephone);
	    /*获取到总的页码数目*/
	...
	public void listAll(HttpServletResponse response) throws Exception {
		...
		for(Coach coach:coachList) {
			...
			jsonCoach.accumulate("coachId", coach.getCoachId());
	...
	public String frontlist(String name,String birthDate,String telephone,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		...
		if (birthDate == null) birthDate = "";
		if (telephone == null) telephone = "";
		List<Coach> coachList = coachService.queryCoach(name, birthDate, telephone, currentPage);
	    /*计算总的页数和总的记录数*/
	    coachService.queryTotalPageAndRecordNumber(name, birthDate, telephone);
	    /*获取到总的页码数目*/
	    int totalPage = coachService.getTotalPage();
     ...
     /*前台查询Coach信息*/
	@RequestMapping(value="/{coachId}/frontshow",method=RequestMethod.GET)
	public String frontshow(@PathVariable Integer coachId,Model model,HttpServletRequest request) throws Exception {
		/*根据主键coachId获取Coach对象*/
	...
	/*ajax方式显示教练修改jsp视图页*/
	@RequestMapping(value="/{coachId}/update",method=RequestMethod.GET)
	public void update(@PathVariable Integer coachId,Model model,HttpServletRequest request,HttpServletResponse response) throws Exception {
        /*根据主键coachId获取Coach对象*/
        Coach coach = coachService.getCoach(coachId);
	...
	/*ajax方式更新教练信息*/
	@RequestMapping(value = "/{coachId}/update", method = RequestMethod.POST)
    ...
    /*删除教练信息*/
	@RequestMapping(value="/{coachId}/delete",method=RequestMethod.GET)
	public String delete(@PathVariable Integer coachId,HttpServletRequest request) throws UnsupportedEncodingException {
		  try {
	...
	public void delete(String coachIds,HttpServletRequest request,HttpServletResponse response) throws IOException, JSONException {
		String message = "";
    	boolean success = false;
        try { 
        	int count = coachService.deleteCoachs(coachIds);
        	success = true;
	...
	public void OutToExcel(String name,String birthDate,String telephone, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        ...
        if(birthDate == null) birthDate = "";
        if(telephone == null) telephone = "";
        List<Coach> coachList = coachService.queryCoach(name,birthDate,telephone);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Coach信息记录"; 
        String[] headers = { "姓名","性别","教练照片","出生日期","联系电话","邮箱"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<coachList.size();i++) {
        	Coach coach = coachList.get(i); 
        	dataset.add(new String[]{coach.getName(),coach.getSex(),coach.getCoachPhoto(),coach.getBirthDate(),coach.getTelephone(),coach.getEmail()});
        }
        /*
