...
public class AnswerService {
    ...
    /*按照查询条件分页查询学员答案记录*/
    public ArrayList<Answer> queryAnswer(Paper paperObj,UserInfo userObj,String addTime,int currentPage) throws Exception { 
     	String where = "where 1=1";
    	if(null != paperObj && paperObj.getPaperId()!= null && paperObj.getPaperId()!= 0)  where += " and t_answer.paperObj=" + paperObj.getPaperId();
    	if(null != userObj &&  userObj.getUser_name() != null  && !userObj.getUser_name().equals(""))  where += " and t_answer.userObj='" + userObj.getUser_name() + "'";
    	if(!addTime.equals("")) where = where + " and t_answer.addTime like '%" + addTime + "%'";
    	int startIndex = (currentPage-1) * this.rows;
    	return answerMapper.queryAnswer(where, startIndex, this.rows);
    }

    /*按照查询条件查询所有记录*/
    public ArrayList<Answer> queryAnswer(Paper paperObj,UserInfo userObj,String addTime) throws Exception  { 
     	String where = "where 1=1";
    	if(null != paperObj && paperObj.getPaperId()!= null && paperObj.getPaperId()!= 0)  where += " and t_answer.paperObj=" + paperObj.getPaperId();
    	if(null != userObj &&  userObj.getUser_name() != null && !userObj.getUser_name().equals(""))  where += " and t_answer.userObj='" + userObj.getUser_name() + "'";
    	if(!addTime.equals("")) where = where + " and t_answer.addTime like '%" + addTime + "%'";
    ...
    /*当前查询条件下计算总的页数和记录数*/
    public void queryTotalPageAndRecordNumber(Paper paperObj,UserInfo userObj,String addTime) throws Exception {
     	String where = "where 1=1";
    	if(null != paperObj && paperObj.getPaperId()!= null && paperObj.getPaperId()!= 0)  where += " and t_answer.paperObj=" + paperObj.getPaperId();
    	if(null != userObj &&  userObj.getUser_name() != null && !userObj.getUser_name().equals(""))  where += " and t_answer.userObj='" + userObj.getUser_name() + "'";
    	if(!addTime.equals("")) where = where + " and t_answer.addTime like '%" + addTime + "%'";
